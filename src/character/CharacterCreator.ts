...
export class CharacterCreator {
    ...
    async getRandomCharacter(numberOfSpells = 4, numberOfFeatures = 2): Promise<Character> {
    ...
    private async getRandomSpells(classIndex: string, numberOfSpells: number = 4): Promise<Response> {
        const spells = await this.spellService.getClassSpells(classIndex);
        const randomSpells: Response = {
            count: 0,
            results: []
        }
        const amountOfSpells = spells.count > numberOfSpells ? numberOfSpells : spells.count;
    ...
    private async getRandomFeatures(classIndex: string, numberOfFeatures: number = 4): Promise<Response> {
        ...
        const amountOfFeatures = features.count > numberOfFeatures ? numberOfFeatures : features.count;
